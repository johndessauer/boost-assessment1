// ── Scoring logic (inlined — no import needed) ────────────────────────────────
const profileLabels = {
...
export default async (req) => {
  ...
  try {
    ...
    const reportHtml = reportText.split('\n').map(l => l.startsWith('SECTION') ? `<h3 style="color:#1A5C38;margin:20px 0 8px">${l}</h3>` : l.trim() ? `<p style="margin:0 0 10px;line-height:1.7">${l}</p>` : '<br>').join('')
